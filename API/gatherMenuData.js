...
export async function gatherMenuData() {
    ...
    let inputArrayForObjectifying = [];
    let orderButtons = createMenuItem(restaurantList, inputArrayForObjectifying);
}

function createMenuItem(restaurantList, inputArrayForObjectifying) {
    let menuItemArray = [];
    ...
    restaurantList.forEach((restaurant, restaurantIndex) => {
        ...
        document.body.append(restaurantItem);
        ...
        // inputArrayForObjectifying.append([]);
        specificRestaurant.setAttribute(
            "id",
            `${restaurant.restaurant_name}-restaurant-id`
        );
        specificRestaurant.setAttribute("name", "restaurant-menu");

        restaurantList[restaurantIndex].menus.forEach((menuList, menuListIndex) => {
            restaurantList[restaurantIndex].menus[menuListIndex].menu_sections.forEach(
                (menuSection, menuSectionIndex) => {
                    //console.log(menuSection);
                    //get variable equal to the pertinent menu heading sections
                    let sectionTitle =
                    restaurantList[restaurantIndex].menus[menuListIndex].menu_sections[
                            menuSectionIndex
                        ].section_name;
                    ...
                    restaurantList[restaurantIndex].menus[menuListIndex].menu_sections[
                        menuSectionIndex
                    ].menu_items.forEach((menuItem, menuItemIndex) => {
                        //get variables equal to the pertinent menu item values
                        let dishName =
                        restaurantList[restaurantIndex].menus[menuListIndex].menu_sections[
                                menuSectionIndex
                            ].menu_items[menuItemIndex].name;
                        let dishDesc =
                        restaurantList[restaurantIndex].menus[menuListIndex].menu_sections[
                                menuSectionIndex
                            ].menu_items[menuItemIndex].description;
                        let dishPrice =
                        restaurantList[restaurantIndex].menus[menuListIndex].menu_sections[
                                menuSectionIndex
                            ].menu_items[menuItemIndex].price;
                        ...
                        menuItemArray.append(yesorno);
                        menuItemArray.append(dishName);
                        let quantity = document.createElement("select");
                        quantity.setAttribute("name", "quantity");
                        menuItemArray.append(quantity);
                        menuItemArray.append(dishPrice);
                        inputArrayForObjectifying.append(menuItemArray);
                    ...
                    applyOrderBtn.addEventListener("click", function (event) {
                        let exampleData = makeOrderObjArr(inputArrayForObjectifying);
...
function makeOrderObjArr(arrOfElements) {
    let arr = [];
    arrOfElements.forEach((input, index) => {
        let foodItemArray = input;
        if(foodItemArray[0].is(":checked")) {
            arr.append({
